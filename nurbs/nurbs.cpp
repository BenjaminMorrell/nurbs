/*=====================================================================
 ...
 Modified by: Ashraful Kadir           (2008-2010)
...
*/
template <class T, int D>
void NurbsCurve<T,D>::basisFuns(T u, int i, Vector<T>& V) const{
  T* left = (T*) alloca(2*(deg_+1)*sizeof(T)) ;
  T* right = &left[deg_+1] ;
  ...
  for(int j=1; j<= deg_ ; j++){
    ...
    saved = 0.0 ;
    for(int r=0 ; r<j; r++){
      temp = V[r]/(right[r+1]+left[j-r]) ;
      V[r] = saved+right[r+1] * temp ;
      saved = left[j-r] * temp ;
    }
    V[j] = saved ;
  }
  ...
  for(degL=1; degL<=degC+1 ; degL++){
	...
	*this = tcurve ;
    ...
    for(i=0;i<Q.n();i++)
    {
      T u_i ;
      Point_nD<T,N> r_i ;
      projectTo(Q[i],ub[i],u_i,r_i,0.001,0.001,maxTry) ;
      ek[i] = norm(r_i-Q[i]) ;
      ub[i] = u_i ;
    }
  }
...
*/
template <class T, int N>
bool NurbsCurve<T,N>::projectTo(const Point_nD<T,N>& p, T guess, T& u,
  Point_nD<T,N>& r, T e1, T e2, int& maxTry) const
{
  T un ;
  T c1, c2;
  T e12 = e1*e1;
...
//-----------------------------------------------------------------------------
/*!
  \brief project a point onto the curve, search within the given boundary
...
template <class T, int N>
bool NurbsCurve<T,N>::projectTo(const Point_nD<T,N>& p, T guess, T& u,
  Point_nD<T,N>& r, T umin, T umax, T e1, T e2,int maxTry) const
{
  T un ;
  T c1, c2;
  T e12 = e1*e1;
...
/*!
  \brief Decompose the curve into B�zier segments

  This function decomposes the curve into an array of 4D B�zier 
  segments.

  \param c  an array of B�zier segments

  \warning The end B�zier segments will not be valid if the NURBS curve 
           is not clamped.
...
// FIXME:     list2 = ca[i].tesselate(tolerance,uk) ;
...
/*!
   \brief decompose the closed curve into B�zier segments
   
   This function decomposes a closed curve into an array of B�zier 
   segments.
...
//-----------------------------------------------------------------------------
/*!
  \brief Writes the curve in the .vtu (VTK) format to a file.
...
*/
template <class T, int N>
int NurbsCurve<T,N>::writeVTU(const char* filename, T step) const {
  ofstream fout(filename);  
  ...
  int point_count = 0;
  for(u = U[deg_]; u < u_max+(step/2.0); u += step) // "<= u_max" doesn't work
	point_count++;

  fout << "<VTKFile type=\"UnstructuredGrid\"  version=\"0.1\"   >\n";
  fout << "<UnstructuredGrid>\n";
  fout << "<Piece  NumberOfPoints=\""<< point_count << "\"  NumberOfCells=\"" << point_count-1 << "\">\n";
  fout << "<Points>\n";
  fout << "<DataArray  type=\"Float64\"  NumberOfComponents=\"3\"  format=\"ascii\">\n";

  for(u = U[deg_]; u < u_max+(step/2.0); u += step) { 
    a2 = this->pointAt(u) ;
	fout << "  " << a2.x() << " " << a2.y() << " " << a2.z() << "\n";
  }
